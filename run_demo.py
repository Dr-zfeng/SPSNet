...
#############################################################################################
parser.add_argument('--model_name', '-m', type=str, default='SPSNet')
parser.add_argument('--weight_name', '-w', type=str, default='SPSNet') # RTFNet_152, RTFNet_50, please change the number of layers in the network file
...
if __name__ == '__main__':
    ...
    #model = DeepLabV3Plus(encoder_name="resnet101",classes=5,in_channels=3)
    #model = eval(args.model_name)(encoder_name="resnet101",classes=args.n_class,in_channels=3)
    ...
    print("* recall per class: \n    unlabeled: %.6f, sat: %.6f, fanban: %.6f, lidar: %.6f, penzui: %.6f" \
    ...
    print("* F1 per class: \n    unlabeled: %.6f, sat: %.6f, fanban: %.6f, lidar: %.6f, penzui: %.6f" \
          %(F1_per_class[0], F1_per_class[1], F1_per_class[2], F1_per_class[3], F1_per_class[4])) 
    print("* iou per class: \n    unlabeled: %.6f, sat: %.6f, fanban: %.6f, lidar: %.6f, penzui: %.6f" \
          %(iou_per_class[0], iou_per_class[1], iou_per_class[2], iou_per_class[3], iou_per_class[4])) 
    
    print("\n* average values (np.mean(x)): \n recall: %.6f, pre: %.6f, F1: %.6f iou: %.6f" \
          %(recall_per_class.mean(), precision_per_class.mean(), F1_per_class.mean(), iou_per_class.mean()))
    print("* average values (np.mean(np.nan_to_num(x))): \n recall: %.6f, pre: %.6f, F1: %.6f iou: %.6f" \
          %(np.mean(np.nan_to_num(recall_per_class)), np.mean(np.nan_to_num(precision_per_class)), np.mean(np.nan_to_num(F1_per_class)), np.mean(np.nan_to_num(iou_per_class))))
